...
heroes.push('wonderwoman'); //['superman', 'batman', 'flash', 'wonderwoman']

// remove 'superman' and store him in a var called firstHero
var firstHero = heroes.shift(); //['batman', 'flash', 'wonderwoman']

// add 'spongebob' to the start of the array
heroes.unshift('spongebob'); //['spongebob', 'batman', 'flash', 'wonderwoman']

// remove 'flash' from the array and store him in a var called secondHero
var secondHero = heroes.splice(2, 1); //['spongebob', 'batman', 'wonderwoman']

// leave batman in the array but put a copy of him on a var called thirdHero
var thirdHero = heroes.slice(1, 2)[0]; //['spongebob', 'batman', 'wonderwoman']
...
function removeItem(stringArray, string) {
	for (var i = 0; i < stringArray.length; i++){
		if (stringArray[i] == string) stringArray.splice(i, 1);
	}
	if (stringArray.indexOf(string) > -1) removeItem(stringArray, string);
	return stringArray;
}
...
// it looks to see if the person’s name is Jeremy and changes the person object
...
// #15 Create a function called outside that takes in a temperature (number), a humidity(number),
// and a cloudiness(number), in that order. Using the following to return the correct values
/*
...
function outside(temperature, humidity, cloudiness) {
	if(temperature > 80 && humidity > 40) return 'I\'m all sweat';
	else if(temperature < 40 && cloudiness > 60) return 'I have icecicles';
	else if(temperature > 80 && humidity < 40 && cloudiness < 20) return 'I\'m literally in the desert';
...
// and invokes it(holla) with the string(back) + ' back'.
